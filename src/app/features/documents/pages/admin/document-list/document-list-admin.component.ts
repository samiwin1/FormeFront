...
export class DocumentListAdminComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.formationService.getAll().subscribe({
      next: (list) => {
        (list ?? []).forEach((f) => {
          if (f.id != null) {
            this.formationTitles.set(f.id, f.title ?? '');
          }
        });
      },
      error: () => {},
  ...
  deleteDocument(doc: AdminDocument): void {
    ...
    if (!this.canDeleteDocument(doc)) {
      this.deleteError = 'Only super admin or the document owner can delete this document.';
      return;
    }

    const requesterId = this.authService.getUserId();
    if (requesterId == null) {
      this.deleteError = 'You must be logged in to delete a document.';
      return;
    }

    const isSuperAdmin = this.authService.isSuperAdmin();
    if (!confirm(`Delete document "${doc.title || doc.fileName}"?`)) {
      return;
    }
